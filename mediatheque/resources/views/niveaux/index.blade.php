...
@section('content')
    <div class="flex flex-col justify-center items-center w-full ml-28 mx-12 space-y-6">
        <h1 class="text-3xl"> Liste des Niveaux </h1>
                ...
                <table class="bg-white w-full">
                    ...
                    @forelse($niveaux as $niveau)
                        <tr class="fieldset_border">
                            <td class="fieldset_border"> {{ $niveau->libelle }} </td>
                            <td class="flex space-x-3 p-2">
                                ...
                                <div>
                                    <form action="{{ route('niveaux.destroy', $niveau) }}" id="form_destroy{{$niveau->id_niveau}}" method="post">
                                        @csrf
                                        @method('delete')
                                        <input type="button" onclick="AlertSwal(title='Attention', text='Voulez vous vraiment supprimer cet niveaux ?', icon='warning', form_tag='form_destroy{{$niveau->id_niveau}}');" value="Supprimer" class="button button_delete">
                                    </form>
                                </div>
                            </td>
                        </tr>
