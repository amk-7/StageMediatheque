...
@section('content')
    <div class="flex flex-col justify-center items-center w-full ml-28 mx-12 space-y-6">
        <h1 class="text-3xl"> Liste des Langues </h1>
                ...
                <table class="bg-white w-full">
                    ...
                    @forelse($langues as $langue)
                        <tr class="fieldset_border">
                            <td class="fieldset_border"> {{ $langue->libelle }} </td>
                            <td class="flex space-x-3 p-2">
                                ...
                                <div>
                                    <form action="{{ route('langues.destroy', $langue) }}" id="form_destroy{{ $langue->id_langue }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <input 
                                            type="button" 
                                            onclick="AlertSwal(
                                                title='Attention',
                                                text='Voulez-vous vraiment supprimer cette langue ?',
                                                icon='warning',
                                                form_tag='form_destroy{{$langue->id_langue }}'
                                            );" 
                                            value="Supprimer" 
