    ...
    <div class="flex flex-col items-center m-auto" id="searchParametersField">
        ...
        <div class="m-3 md:flex md:space-x-3">
            <div class="flex space-x-3">
                <select wire:model="annee_debut" name="annee_parution_debut" class="select_btn w-1/2 mb-3">
                    <option value="">Toute annees </option>
                    @for($a=date('Y'); $a >= $annees; $a--)
                        <option value="{{ $a }}"> {{ $a }} </option>
                    @endfor
                </select>
                <select wire:model="annee_fin" name="annee_parution_fin" class="select_btn w-1/2 mb-3" style="">
                    <option value="">Toute annees</option>
                    @for($a=date('Y'); $a >= $annees; $a--)
                        <option value="{{ $a }}"> {{ $a }} </option>
                    @endfor
            ...
            <div class="flex space-x-3">
                <select wire:model="langue" name="langue" class="select_btn w-full mb-3">
                    <option value="">Toute langues</option>
                    @foreach($langues as $langue)
                        <option value="{{ $langue }}"> {{ $langue }} </option>
                    @endforeach
                </select>
                <select wire:model="type" name="type" class="select_btn w-full mb-3">
            ...
            <div class="flex space-x-3">
                <select wire:model="categorie" name="domaine" class="select_btn w-full mb-3">
                    <option value="">Tous domaines</option>
                    @foreach($categories as $categorie)
                        <option value="{{ $categorie }}"> {{ $categorie }} </option>
                    @endforeach
                </select>
                <select wire:model="niveau" name="niveau" class="select_btn w-full mb-3">
